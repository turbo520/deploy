...
#
#  支持的拓扑：
#    ① VLESS 入站 → 多台 VLESS 下游（负载均衡）
#    ② VLESS 入站 → 多台 Socks5 下游（负载均衡）  ← 新增
#    ③ Socks5 入站 → 直连（本机作 Socks5 代理）
#    ④ 叠加模式：在现有配置上新增 Socks5 入站
#
#  所有中转用户均带 email 字段，流量限额系统可直接识别
# ══════════════════════════════════════════════════════════════════════════════
...
# 交互式收集下游服务器列表，返回两个字符串：
#   OUTBOUNDS_JSON  —— 拼接好的 outbound JSON 数组元素（逗号分隔）
#   BALANCER_TAGS   —— 下游 tag 数组（bash array）
# 下游协议由 $1 决定：vless / socks5
_collect_downstream_servers() {
    local proto="${1:-vless}"
    OUTBOUNDS_JSON=""
    BALANCER_TAGS=()
    local server_count=0

    echo -e "\n${YELLOW}添加下游${proto^^}服务器（直接回车结束添加）:${NC}"
    ...
    while true; do
        ...
        local DS_PORT DS_CREDENTIAL DS_ENTRY DS_TAG
        DS_TAG="downstream-${proto}-${server_count}"

        if [[ "$proto" == "vless" ]]; then
            read -p "  服务器端口 [默认 443]: " DS_PORT; DS_PORT=${DS_PORT:-443}
            read -p "  UUID: " DS_CREDENTIAL
            [[ -z "$DS_CREDENTIAL" ]] && {
                warn "  UUID 不能为空，跳过"; server_count=$((server_count-1)); continue
            }
...
ST
                    ...
                    ;;
                3) ds_stream='"streamSettings": { "network": "tcp" }' ;;
            esac
            ...
            DS_ENTRY=$(cat << OB
    {
      ...
      "settings": {
        "vnext": [{
          "address": "${DS_ADDR}",
          "port": ${DS_PORT},
          "users": [{ "id": "${DS_CREDENTIAL}", ${ds_flow} "encryption": "none" }]
        }]
      },
${ds_stream}
    }
OB
            ...
            read -p "  服务器端口 [默认 1080]: " DS_PORT; DS_PORT=${DS_PORT:-1080}
            local DS_USER DS_PASS DS_AUTH_BLOCK=""
            read -p "  用户名（无认证留空）: " DS_USER
            if [[ -n "$DS_USER" ]]; then
                read -p "  密码: " DS_PASS
                DS_AUTH_BLOCK='"user": "'"$DS_USER"'", "pass": "'"$DS_PASS"'",'
            fi
...
OB
        ...
        OUTBOUNDS_JSON="${OUTBOUNDS_JSON}${DS_ENTRY},"
        BALANCER_TAGS+=("\"${DS_TAG}\"")
        success "  下游服务器 #${server_count} 已添加: ${DS_ADDR}:${DS_PORT}"
    done

    [[ $server_count -eq 0 ]] && return 1
    return 0
}
...
gen_vless_relay_config() {
    step "VLESS 中转（入站 VLESS → 下游 VLESS，支持多台负载均衡）"
    echo -e "${DIM}客户端连本机 VLESS，本机按策略转发到多台下游 VLESS 服务器${NC}\n"

    read -p "中转节点监听端口 [默认 443]: " RELAY_IN_PORT
    RELAY_IN_PORT=${RELAY_IN_PORT:-443}

    read -p "入站用户备注名称 [默认 relay]: " RELAY_REMARK
    RELAY_REMARK=${RELAY_REMARK:-relay}

    local RELAY_UUID; RELAY_UUID=$(xray uuid 2>/dev/null || cat /proc/sys/kernel/random/uuid)
    local RELAY_EMAIL="${RELAY_REMARK}@relay"
    info "入站 UUID:  $RELAY_UUID"
    ...
    # 负载均衡
    echo -e "\n${YELLOW}负载均衡策略:${NC}"
    echo -e "  ${GREEN}1${NC}. 随机  ${GREEN}2${NC}. 轮询"
...
${OUTBOUNDS_JSON}
  ...
  "routing": {
    ...
    "rules": [
      { "type": "field", "inboundTag": ["api-in"], "outboundTag": "api" },
      { "type": "field", "inboundTag": ["relay-vless-in"], "balancerTag": "relay-balancer" },
      { "type": "field", "ip": ["geoip:private"], "outboundTag": "block" }
    ]
  }
}
EOF
    ...
    systemctl enable xray > /dev/null 2>&1
    systemctl restart xray && sleep 2
    systemctl is-active --quiet xray && success "VLESS 中转节点启动成功" || error "启动失败，请查看日志"

    local PUB_IP; PUB_IP=$(curl -s4 ip.sb || curl -s6 ip.sb)

    echo ""
    echo -e "${BLUE}═══════════════════════════════════════════════════${NC}"
    echo -e "${BLUE}          VLESS → VLESS 中转节点信息${NC}"
    echo -e "${BLUE}═══════════════════════════════════════════════════${NC}"
    echo -e "${YELLOW}入站地址:${NC}   ${PUB_IP}:${RELAY_IN_PORT}"
    echo -e "${YELLOW}UUID:${NC}       ${RELAY_UUID}"
    echo -e "${YELLOW}Email:${NC}      ${RELAY_EMAIL}"
    [[ "$IN_SEC" == "1" ]] && {
        echo -e "${YELLOW}PublicKey:${NC}  ${IN_PUBKEY}"
        echo -e "${YELLOW}ShortId:${NC}   ${IN_SHORTID}"
        echo -e "${YELLOW}SNI:${NC}       ${IN_DEST}"
    }
    [[ "$IN_SEC" == "2" ]] && echo -e "${YELLOW}域名:${NC}      ${IN_DOMAIN}"
    echo -e "${YELLOW}下游数量:${NC}  ${#BALANCER_TAGS[@]}"
    echo -e "${YELLOW}负载策略:${NC}  ${lb_strategy}"
    echo -e "${BLUE}═══════════════════════════════════════════════════${NC}"
    tip "用户 email「${RELAY_EMAIL}」可在「流量限额管理」中设置配额"
...
==========================================
生成时间:  $(date "+%Y-%m-%d %H:%M:%S")
入站地址:  ${PUB_IP}:${RELAY_IN_PORT}
UUID:      ${RELAY_UUID}
Email:     ${RELAY_EMAIL}
PublicKey: ${IN_PUBKEY:-N/A}
ShortId:   ${IN_SHORTID:-N/A}
SNI:       ${IN_DEST:-N/A}
域名:      ${IN_DOMAIN:-N/A}
下游数量:  ${#BALANCER_TAGS[@]}
负载策略:  ${lb_strategy}
==========================================
EOF
    info "配置摘要已保存至 $CLIENT_FILE"

    # 询问是否立即设置流量配额
    echo ""
    read -p "是否立即为入站用户「${RELAY_EMAIL}」设置流量配额? [y/N]: " set_q
    [[ "$set_q" == "y" || "$set_q" == "Y" ]] && quota_set
}

# ─────────────────────────── ② VLESS → Socks5 中转 ──────────────────────────

gen_vless_to_socks5_relay() {
    step "VLESS → Socks5 中转（入站 VLESS → 下游 Socks5，支持多台负载均衡）"
    echo -e "${DIM}客户端连本机 VLESS，本机通过 Socks5 出站访问目标（常用于落地机场/企业代理）${NC}\n"

    read -p "中转节点监听端口 [默认 443]: " RELAY_IN_PORT
    RELAY_IN_PORT=${RELAY_IN_PORT:-443}

    read -p "入站用户备注名称 [默认 relay-s5]: " RELAY_REMARK
    RELAY_REMARK=${RELAY_REMARK:-relay-s5}
...
${OUTBOUNDS_JSON}
  ...
  "routing": {
    ...
    "rules": [
      { "type": "field", "inboundTag": ["api-in"], "outboundTag": "api" },
      { "type": "field", "inboundTag": ["relay-vless-s5-in"], "balancerTag": "s5-balancer" },
      { "type": "field", "ip": ["geoip:private"], "outboundTag": "block" }
    ]
  }
}
EOF
    ...
    systemctl enable xray > /dev/null 2>&1
    systemctl restart xray && sleep 2
    systemctl is-active --quiet xray && success "VLESS→Socks5 中转节点启动成功" || error "启动失败，请查看日志"

    local PUB_IP; PUB_IP=$(curl -s4 ip.sb || curl -s6 ip.sb)

    echo ""
    echo -e "${BLUE}═══════════════════════════════════════════════════${NC}"
    echo -e "${BLUE}         VLESS → Socks5 中转节点信息${NC}"
    echo -e "${BLUE}═══════════════════════════════════════════════════${NC}"
    echo -e "${YELLOW}入站地址:${NC}   ${PUB_IP}:${RELAY_IN_PORT}（客户端连此处）"
    echo -e "${YELLOW}UUID:${NC}       ${RELAY_UUID}"
    echo -e "${YELLOW}Email:${NC}      ${RELAY_EMAIL}"
    [[ "$IN_SEC" == "1" ]] && {
        echo -e "${YELLOW}PublicKey:${NC}  ${IN_PUBKEY}"
    ...
    [[ "$IN_SEC" == "2" ]] && echo -e "${YELLOW}域名:${NC}      ${IN_DOMAIN}"
    echo -e "${YELLOW}下游 Socks5:${NC} ${#BALANCER_TAGS[@]} 台"
    echo -e "${YELLOW}负载策略:${NC}  ${lb_strategy}"
    echo -e "${BLUE}═══════════════════════════════════════════════════${NC}"
    tip "用户 email「${RELAY_EMAIL}」可在「流量限额管理」中设置配额"
...
gen_socks5_config() {
    step "Socks5 代理入站（本机对外提供 Socks5 代理）"
    echo -e "${DIM}本机直接开放 Socks5 端口，支持多用户账号及流量限额${NC}\n"
    ...
    read -p "监听地址 [默认 0.0.0.0，填 127.0.0.1 仅本机]: " S5_LISTEN
    S5_LISTEN=${S5_LISTEN:-"0.0.0.0"}

    # 多用户账号管理
    local S5_ACCOUNTS_JSON="" S5_AUTH=false
    read -p "是否启用账号密码认证（支持多用户）? [y/N]: " auth_yn
    if [[ "$auth_yn" =~ ^[Yy]$ ]]; then
        S5_AUTH=true
        local u_count=0
        ...
        while true; do
            ...
            [[ -n "$S5_ACCOUNTS_JSON" ]] && S5_ACCOUNTS_JSON="${S5_ACCOUNTS_JSON},"
            S5_ACCOUNTS_JSON="${S5_ACCOUNTS_JSON}{\"user\":\"${S5_USER}\",\"pass\":\"${S5_PASS}\"}"
            success "  用户 ${S5_USER} 已添加"
        done
        [[ $u_count -eq 0 ]] && { warn "未添加任何用户，改为无认证模式"; S5_AUTH=false; }
    fi

    local UDP_SUPPORT=true
    read -p "启用 UDP 支持? [Y/n]: " udp_yn; udp_yn=${udp_yn:-Y}
    [[ "$udp_yn" =~ ^[Nn]$ ]] && UDP_SUPPORT=false
...
SINB
    ...
    if $MERGE; then
        # 若已有同 tag 的 socks5-in，先删除再追加（避免重复）
        jq 'del(.inbounds[] | select(.tag=="socks5-in"))' \
            "$XRAY_CONFIG" > /tmp/xray_tmp.json && mv /tmp/xray_tmp.json "$XRAY_CONFIG"
...
EOF
    ...
    xray run -test -config "$XRAY_CONFIG" > /dev/null 2>&1 || error "配置验证失败"
    ...
    echo -e "${YELLOW}认证:${NC}  $($S5_AUTH && echo "账号密码" || echo "无需认证")"
    $S5_AUTH && echo -e "${YELLOW}用户数:${NC} ${u_count}"
...
地址:     ${PUB_IP}
端口:     ${S5_PORT}
认证:     $($S5_AUTH && echo "账号密码（共 ${u_count} 个用户）" || echo "无需认证")
UDP:      ${UDP_SUPPORT}
==========================================
EOF

    # Socks5 无 email 字段，不对接流量限额（提示用户）
    $S5_AUTH && tip "Socks5 账号不支持 Xray 流量统计，如需限额请改用 VLESS 中转模式"
}
...
manage_relay_socks5() {
    while true; do
        ...
        echo -e "${PURPLE}═══════════════════════════════════════════════${NC}"
        echo -e "  ${BOLD}中转节点（支持流量限额）${NC}"
        echo -e "  ${GREEN}1${NC}. VLESS 入站 → 多台 VLESS 下游（负载均衡）"
        echo -e "  ${GREEN}2${NC}. VLESS 入站 → 多台 Socks5 下游（负载均衡）"
        echo -e "  ${BOLD}Socks5 代理${NC}"
        echo -e "  ${GREEN}3${NC}. 本机开放 Socks5 代理（支持多用户账号）"
        echo -e "  ${BOLD}管理${NC}"
        echo -e "  ${GREEN}4${NC}. 查看当前中转/Socks5 入站状态"
        echo -e "  ${GREEN}5${NC}. 为中转用户设置流量配额"
...
_view_relay_status() {
    ...
    echo ""
    echo -e "${BLUE}═══════════════════════════════════════════════════${NC}"
    echo -e "${BLUE}           当前入站配置（中转 & Socks5）${NC}"
    echo -e "${BLUE}═══════════════════════════════════════════════════${NC}"

    # 列出所有入站
    jq -r '.inbounds[] | select(.tag != "api-in") |
        "  [\(.tag)]  协议: \(.protocol)  端口: \(.port)  " +
        if .settings.clients then
            "用户数: \(.settings.clients | length)"
        elif .settings.servers then
            "下游: \(.settings.servers | length) 台"
        elif .settings.accounts then
            "账号数: \(.settings.accounts | length)"
        else "" end' "$XRAY_CONFIG" 2>/dev/null

    echo ""
    # 列出出站/下游
    local downstream_count
    downstream_count=$(jq '[.outbounds[] | select(.tag | startswith("downstream"))] | length' \
        "$XRAY_CONFIG" 2>/dev/null)
    echo -e "  下游服务器总数: ${downstream_count:-0}"

    # 负载均衡器
    local balancer_info
    balancer_info=$(jq -r '.balancers[]? | "  负载均衡[\(.tag)]: \(.strategy.type) 策略，选择器: \(.selector | join(","))"' \
        "$XRAY_CONFIG" 2>/dev/null)
    [[ -n "$balancer_info" ]] && echo -e "$balancer_info"
...
install_new() {
    ...
    echo -e "  ${GREEN}2${NC}. VLESS 中转 → 多台 VLESS 下游（负载均衡）"
    echo -e "  ${GREEN}3${NC}. VLESS 中转 → 多台 Socks5 下游（负载均衡）"
    ...
    case $INSTALL_TYPE in
        ...
        3) gen_vless_to_socks5_relay;    enable_bbr; return ;;
